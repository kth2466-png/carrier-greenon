...
> Carrier Jump Aurora 개편 (2026-08-12): 전체 페이지 배경에 `#0096A5 → #4EBDD5 → #9BC6C0 → #B6DBE5 → #D7B4D2` 세로 그라데이션을 적용하고, Azure Blue `#265AFC`는 버튼·활성·포인트에, Sunglow `#FFDA38`은 리워드·배지·완료 강조에 사용했습니다. 메인 문구를 `Carrier Jump 1기 화이팅!`으로 변경하고 제공된 `피규어 팔짱.png`를 900px WebP 메인 캐릭터로 최적화했습니다. 기존 Supabase 구조에 100·200·300·500P 미션과 1,500·3,000·5,000·8,000P 리워드를 연결했으며 인증 사용자 전용 RPC와 유니크 제약으로 중복 적립을 차단했습니다. Playwright는 360px·390px·768px·1024px 37개 레이아웃/접근성/모션 검사를 모두 통과했고, 기존 3개 해상도 각 47개 회귀 클릭도 통과했습니다. 모의 Supabase 통합 흐름에서 2,000P → 미션 완료 2,100P, 새로고침 후 2,100P 유지, 3,000P 상품 비활성, 1,500P 구매 후 600P 및 주문 기록을 확인했습니다.
